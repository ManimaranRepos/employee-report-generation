    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/5">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-fuchsia-500/20 grid place-items-center">
            <Users className="w-3.5 h-3.5 text-fuchsia-300" />
          </div>
          <div>
            <div className="text-sm font-semibold text-slate-100">Review & Send</div>
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-white/5">
        {employees.length === 0 ? (
          ...
          employees.map((emp) => {
            ...
            return (
              <div key={emp.EmpID} className="flex items-start gap-3 px-4 py-3.5 hover:bg-slate-800/30 transition-colors group">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-slate-100 truncate">{emp.FullName}</div>
                  <div className="text-[11px] text-slate-400 truncate mt-0.5">
                    {emp.Department} · {emp.Designation}
                  </div>
                  <div className="flex items-center gap-1 mt-1">
                    <Mail className="w-3 h-3 text-indigo-400 shrink-0" />
                    <span className="text-[11px] text-indigo-300 truncate font-mono">
                      {emp.Email || <span className="text-amber-400 font-sans">No email on file</span>}
                    </span>
                  </div>
                ...
                <button
                  ...
                  className="p-1 rounded-lg hover:bg-slate-700 text-slate-500 hover:text-slate-200 transition-colors opacity-0 group-hover:opacity-100 mt-0.5 shrink-0"
      ...
      {/* Footer — Send All */}
      <div className="px-4 py-3.5 border-t border-white/5 space-y-2">
